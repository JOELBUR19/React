...
export function LoopsDemo() {
  ...
  function ajustarStock() {
    ...
    while (i < cloned.length) {
      //Ciclo que permite recorrer el array (Parece parecido al ciclo burbuja)
  ...
  for (const k in items) {
    //No entendi esta parte, parece estar recorriendo el array
    const idx = Number(k);
  ...
  return (
    <div>
      ...
      <p>
        Promedio de precio (for...of en efecto): <strong>${avg}</strong>
      </p>
      ...
      <p>
        Caros (≥ 60): <strong>{caros.length}</strong> | Total precios (reduce):{" "}
